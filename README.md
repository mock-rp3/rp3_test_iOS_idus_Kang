...
## 21.09.10
+ 새상품 탭 페이지 UI 작업 100% 완료
  - GET API까지 엮어서 상품 데이터 전달 완료

***

## 21.09.09
+ 새상품 탭 페이지 UI 구성 50%
  - 오늘은 회사면접에 수료식까지 있었어서 시간이 없는터라 일단 UI만 올려놓음.

***

## 21.09.08
+ 결제 페이지 UI 작업 100% 완료
  - 캡처 이미지로 넣어둔 회원정보 셀도 완성
+ array 안에 있는 이미지 url for 0...array.count로 했던 구문 while let으로 바꾸려했다가, for item in array로 간단히 해결함. 코드 더 간결해짐. 대신 정보 받아오는 속도가 좀 더 느려진 것 같음.
  - 0번째 셀 누르면 1번 인덱스를 가져와서, 한번 곤혹 겪음.
  - 1번 인덱스 상품 빼고는 더미 데이터 이미지들이 1개밖에 없어서, 처음에 이미지 최대 개수 맞춰서 짜놨던 코드들 싹 다 맞춤형으로 손봄.

***

## 21.09.07
+ 결제 페이지 UI 작업 50% 완료
  - 회원정보 셀은 캡처이미지로 일단 대체하고, 최종 금액 부분만 UI 만들어서, 전 페이지에서 정보 받아온 것 반영

***

## 21.09.06
+ 결제 페이지 결제 기능 POST API 작업 완료
...
## 21.09.05
+ 장바구니 페이지 작업 완료
  - 필요한 작품 주문 정보들 유저디폴트로 이전 페이지들에서 저장
