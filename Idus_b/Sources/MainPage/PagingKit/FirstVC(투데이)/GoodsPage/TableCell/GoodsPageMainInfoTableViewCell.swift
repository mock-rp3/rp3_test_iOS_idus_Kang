...
class GoodsPageMainInfoTableViewCell: UITableViewCell {
    ...
    func configureAuthor(with models: [GoodsPageResponseResult]) {
        ...
        let firstSellNumResult = numberFormatter.string(from: NSNumber(value: models[0].workInfo.price))! + "원"
        firstSellNum.text = firstSellNumResult
        ...
        let sellNumResult = numberFormatter.string(from: NSNumber(value: sellNum0))! + "원"
        sellNum.text = sellNumResult
